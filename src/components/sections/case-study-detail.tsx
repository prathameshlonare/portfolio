...
export function CaseStudyDetail({
  ...
  return (
    <div id={id} className="w-full max-w-7xl mx-auto my-12 scroll-mt-24">
      <NeoCard variant="orange" className="p-6 md:p-10">
        {/* Header Bar */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-3 border-[#1A1A2E] pb-6 mb-8 gap-4">
          <div>
            ...
            <h2 className="text-3xl md:text-5xl font-black text-[#1A1A2E] tracking-tight">{title}</h2>
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 p-3 pt-0">
                {images.map((img, idx) => (
                  <button
                    ...
                    className="border-2 border-[#1A1A2E] shadow-[2px_2px_0px_#FF6B35] hover:shadow-[4px_4px_0px_#FF6B35] transition-all cursor-pointer bg-[#FAFAFA] overflow-hidden aspect-video hover:-translate-x-0.5 hover:-translate-y-0.5"
                  >
                    <img
                      src={img.src}
                      alt={img.alt}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-8">
          ...
          <div className="lg:col-span-6 flex flex-col gap-6">
            {/* Problem */}
            <div className="border-2 border-[#1A1A2E] bg-[#FAFAFA] p-5 shadow-[4px_4px_0px_#1A1A2E]">
              <div className="flex items-center gap-2 mb-2">
                <AlertTriangle className="w-4 h-4 text-[#FF6B35]" />
                <MonoLabel className="text-[#FF6B35]">01 / THE PROBLEM</MonoLabel>
              </div>
              <p className="text-base text-zinc-700 font-medium leading-relaxed">{problem}</p>
            </div>

            {/* Role */}
            <div className="border-2 border-[#1A1A2E] bg-white p-5 shadow-[4px_4px_0px_#7C3AED]">
              <div className="flex items-center gap-2 mb-2">
                <Terminal className="w-4 h-4 text-[#7C3AED]" />
                <MonoLabel className="text-[#7C3AED]">02 / MY ROLE & OWNERSHIP</MonoLabel>
              </div>
              <p className="text-base text-zinc-700 font-medium leading-relaxed">{role}</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col gap-6">
            {/* Method */}
            <div className="border-2 border-[#1A1A2E] bg-white p-5 shadow-[4px_4px_0px_#1A1A2E]">
              <div className="flex items-center gap-2 mb-3">
                <Cpu className="w-4 h-4 text-[#1A1A2E]" />
                <MonoLabel className="text-[#1A1A2E]">03 / METHOD & ARCHITECTURE</MonoLabel>
              </div>
              <ul className="flex flex-col gap-2 font-mono text-xs text-zinc-700 font-bold">
                {method.map((item, idx) => (
            ...
            {/* Outcome */}
            <div className="border-2 border-[#1A1A2E] bg-[#FAFAFA] p-5 shadow-[4px_4px_0px_#FF6B35]">
              <div className="flex items-center gap-2 mb-3">
                <CheckCircle className="w-4 h-4 text-emerald-600" />
                <MonoLabel className="text-emerald-700">04 / MEASURABLE OUTCOMES</MonoLabel>
              </div>
              <ul className="flex flex-col gap-2 font-mono text-xs text-zinc-800 font-extrabold">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-4 border-t-2 border-[#1A1A2E] mb-6">
          ...
          {tech.map((t) => (
            <span
              key={t}
              className="font-mono text-xs font-bold text-[#1A1A2E] bg-[#FAFAFA] border-2 border-[#1A1A2E] px-3 py-1 shadow-[2px_2px_0px_#FF6B35]"
            >
              {t}
            </span>
          ))}
      ...
      {lightboxIndex !== null && createPortal(
        ...
        >
          ...
          <img
            ...
            className="max-w-[90vw] max-h-[85vh] object-contain border-2 border-[#1A1A2E]"
